...
export const resumeBehavior = async (
  ...
  try {
    ...
    try {
      ...
      const { robotId, error: resumeError } = error as {
        robotId: string;
        error: Error;
      };
      console.error("Error in resumeBehavior:", error);
      res.status(500).json({
        success: false,
        message: `Error resuming behavior for robot ID ${robotId}: ${resumeError.message}`,
      });
    }
  } catch (error) {
    console.error("Error in resumeBehavior:", error);
